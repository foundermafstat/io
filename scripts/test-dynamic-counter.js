	...
	try {
		...
		for (const testCase of testCases) {
			const response = await fetch(
				`${BASE_URL}/api/properties?limit=${testCase.limit}`
			);
			const data = await response.json();

			if (response.ok) {
				console.log(
					`   Лимит ${testCase.limit}: найдено ${
						data.properties?.length || 0
					} объектов, всего ${data.totalCount || 0}`
				);
			} else {
		...
		const testTranslations = [
			{
				key: 'properties.foundCount',
				params: { count: '1,234' },
				expected: 'Found properties: 1,234',
			},
			{
				key: 'properties.foundCount',
				params: { count: '0' },
				expected: 'Found properties: 0',
			},
			{
				key: 'properties.foundCount',
				params: { count: '999,999' },
				expected: 'Found properties: 999,999',
			},
		];
