...
export const sendMessageToReplicaDirect = async (
  ...
  try {
    ...
    if (!userId) {
      ...
      throw new Error('User ID is required for API authentication');
    }
    
    // Используем наш API роут вместо прямого вызова к Sensay API
    const apiUrl = '/api/sensay/chat/completions';
    console.log('Using internal API route:', apiUrl);
    
    // Make the API request to our internal route
