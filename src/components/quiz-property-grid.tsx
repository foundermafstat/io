...
export function QuizPropertyGrid({
	...
	if (loading) {
		return (
			<div
				className={`space-y-4 ${className} animate-in fade-in-50 duration-300`}
			>
				...
				<div className="relative">
					...
					<div className="overflow-x-auto scrollbar-hide carousel-container">
						<div
							className="flex gap-4 pb-8 property-grid property-grid-loading"
	...
	if (properties.length === 0) {
		return (
			<div
				className={`text-center py-8 ${className} property-grid-empty animate-in fade-in-50 duration-300`}
			>
				<div className="text-muted-foreground">
					<MapPin className="w-12 h-12 mx-auto mb-4 opacity-50" />
					<h3 className="text-lg font-semibold mb-2">
						{t('quiz.properties.noProperties')}
					</h3>
					<p className="text-sm">
						{currentStep <= 2
	...
	return (
		<div className={`space-y-4 ${className}`}>
			<div className="flex items-center justify-between animate-in fade-in-50 duration-300">
				<div>
					...
					<p className="text-sm text-muted-foreground">
						{properties.length}{' '}
						{properties.length === 1
							? t('quiz.properties.property')
							: t('quiz.properties.properties')}{' '}
						{t('quiz.properties.found')}
					</p>
				</div>
				<Badge variant="outline" className="text-xs">
					{t('quiz.navigation.step')} {currentStep + 1}{' '}
					{t('quiz.properties.found')}
				</Badge>
			</div>
			...
			<div className="relative">
				...
				>
					<div
						className="flex gap-4 pb-8 property-grid property-grid-loaded"
						style={{ width: 'max-content' }}
					>
...
function PropertyCard({ property }: { property: Property }) {
	...
	const priceLabel =
		property.operationType === 'RENT' ? t('quiz.properties.perMonth') : '';
		...
		>
			...
			<div className="relative h-48 bg-muted">
				{!imageError ? (
					<Image
						...
						className={`object-cover property-image ${
							imageLoading ? 'property-image-loading' : 'property-image-loaded'
						}`}
						placeholder="blur"
				...
				<div className="absolute top-2 right-2">
					<Badge
						variant={property.operationType === 'RENT' ? 'default' : 'outline'}
					>
						{property.operationType === 'RENT'
							? t('quiz.properties.rent')
							: t('quiz.properties.sale')}
					</Badge>
				</div>
			</div>
			...
			<CardContent className="p-4">
				...
				<div className="flex items-center justify-between text-xs text-muted-foreground mt-2 pt-2 border-t">
					<span>
						{property.views || 0} {t('quiz.properties.views')}
					</span>
					<span>
						{Array.isArray(property.reviews) ? property.reviews.length : 0}{' '}
						{t('quiz.properties.reviews')}
					</span>
				</div>
			</CardContent>
		</Card>
