	...
	return (
		<Card className="w-full">
			<CardHeader className="text-center pb-4">
				...
				<CardDescription>{t('quiz.intro.subtitle')}</CardDescription>
			</CardHeader>
			<CardContent className="space-y-4">
				<div className="grid grid-cols-1 md:grid-cols-2 gap-3">
					<Card className="p-3 border-2 border-dashed">
						<h3 className="font-semibold mb-2 text-sm">
							{t('quiz.intro.discover.title')}
						</h3>
						<ul className="text-xs space-y-1">
							{getTranslation('quiz.intro.discover.items').map(
					...
					<Card className="p-3 border-2 border-dashed">
						<h3 className="font-semibold mb-2 text-sm">
							{t('quiz.intro.aiPowered.title')}
						</h3>
						<ul className="text-xs space-y-1">
							{(getTranslation('quiz.intro.aiPowered.items') as string[]).map(
								(item: string, index: number) => (
									<li key={index}>• {item}</li>
								)
							)}
...
function PurposeStep({
	...
	onChoice: (purpose: 'rent' | 'buy') => void;
}) {
		...
		<Card className="w-full">
			<CardHeader className="text-center pb-4">
				<CardTitle className="text-xl">
					{t('quiz.steps.purpose.title')}
				</CardTitle>
				<CardDescription>{t('quiz.steps.purpose.subtitle')}</CardDescription>
			</CardHeader>
			<CardContent>
				<div className="grid grid-cols-1 md:grid-cols-2 gap-3">
					<Card
					...
					>
						<div className="text-center space-y-2">
							<h3 className="text-lg font-semibold">
								{t('quiz.steps.purpose.rent.title')}
							</h3>
					...
					>
						<div className="text-center space-y-2">
							<h3 className="text-lg font-semibold">
								{t('quiz.steps.purpose.buy.title')}
							</h3>
							<p className="text-xs text-muted-foreground">
...
function BudgetStep({
	...
	return (
		<Card className="w-full">
			<CardHeader className="text-center pb-4">
				<CardTitle className="text-xl">
					{t('quiz.steps.budget.title')}
				</CardTitle>
				<CardDescription>{t('quiz.steps.budget.subtitle')}</CardDescription>
			</CardHeader>
			<CardContent>
		...
		<Card className="w-full">
			<CardHeader className="text-center">
				<CardTitle className="text-2xl">
					{t('quiz.steps.propertyType.title')}
				</CardTitle>
				<CardDescription>
...
function LocationStep({
	...
	return (
		<Card className="w-full">
			<CardHeader className="text-center pb-4">
				<CardTitle className="text-xl">
					{t('quiz.steps.location.title')}
				</CardTitle>
				<CardDescription>{t('quiz.steps.location.subtitle')}</CardDescription>
			</CardHeader>
			<CardContent>
...
function FeaturesStep({
	...
	return (
		<Card className="w-full">
			<CardHeader className="text-center pb-4">
				<CardTitle className="text-xl">
					{t('quiz.steps.features.title')}
				</CardTitle>
				<CardDescription>{t('quiz.steps.features.subtitle')}</CardDescription>
			</CardHeader>
			<CardContent>
				<div className="grid grid-cols-2 md:grid-cols-4 gap-2">
...
function ResultsStep({
	...
	return (
		<Card className="w-full">
			<CardHeader className="text-center pb-4">
				<CardTitle className="text-xl">
					{t('quiz.steps.results.title')}
				</CardTitle>
				<CardDescription>{t('quiz.steps.results.subtitle')}</CardDescription>
			</CardHeader>
			<CardContent className="space-y-4">
				<div className="grid grid-cols-1 md:grid-cols-2 gap-3">
					<div className="space-y-1">
						<h3 className="font-semibold text-sm">
							{t('quiz.steps.results.purpose')}
						</h3>
					...
					<div className="space-y-1">
						<h3 className="font-semibold text-sm">
							{t('quiz.steps.results.budget')}
						</h3>
						<Badge variant="outline" className="text-sm p-1">
							{preferences.budget
								? `€${preferences.budget.min}-€${preferences.budget.max}`
								: t('quiz.steps.results.notSpecified')}
						</Badge>
					</div>
					<div className="space-y-1">
						<h3 className="font-semibold text-sm">
							{t('quiz.steps.results.propertyTypes')}
						</h3>
				...
				<div className="space-y-1">
					...
					<div className="flex flex-wrap gap-1">
						{preferences.features?.map((feature: string) => (
							...
							<Badge variant="outline" className="text-xs">
								{t('quiz.steps.results.noFeatures')}
							</Badge>
						)}
					</div>
				</div>
				...
				<div className="flex gap-2 justify-center">
					...
					<Button size="sm">{t('quiz.steps.results.searchProperties')}</Button>
				</div>
			</CardContent>
		</Card>
	);
	...
	const loadProperties = React.useCallback(async () => {
		...
		try {
			...
			if (
				...
				state.filters.operationType !== 'both'
			) {
			...
			if (filters.minRentPrice)
				searchParams.set('minPrice', filters.minRentPrice.toString());
			if (filters.maxRentPrice)
				searchParams.set('maxPrice', filters.maxRentPrice.toString());
			if (filters.minSalePrice)
				searchParams.set('minPrice', filters.minSalePrice.toString());
			if (filters.maxSalePrice)
				searchParams.set('maxPrice', filters.maxSalePrice.toString());
	...
	const renderCurrentStep = () => {
		const stepContent = (() => {
			switch (currentStepIndex) {
				...
				case 2:
					return (
						<BudgetStepWrapper budgetRanges={dynamicFilters.budgetRanges} />
					);
				case 3:
					return (
						<PropertyTypeStepWrapper
							propertyTypes={dynamicFilters.propertyTypes}
						/>
					);
				case 4:
		...
		return <div className="w-full">{stepContent}</div>;
	};
		...
		<div className="h-full bg-background flex flex-col overflow-hidden">
			...
			<div className="flex-1 flex flex-col overflow-hidden">
				...
				<div className="flex-1 w-full px-4 py-4 overflow-hidden">
					<div className="h-full flex items-end justify-center pb-2 relative z-50">
						<div className="w-full max-w-3xl mx-auto relative z-50 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 border rounded-lg shadow-lg">
							{renderCurrentStep()}
						</div>
					</div>
				</div>
